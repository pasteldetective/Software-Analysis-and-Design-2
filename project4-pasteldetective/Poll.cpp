 ...
 * @file Network.hpp
 * @author Jackie Yee
 * @brief Project 4: Poll.cpp
 * @version 0.5
 * @date 2022-10-25
 * 
 *Poll class file includes constructors destructors and functions
 **/
#include "Poll.hpp"
...
bool Poll::votePoll(const int& poll_option){
    if (poll_option < int(poll_options_.size())){
...
void Poll::changePollOption(const std::string& new_poll, const int& poll_option){
    if(int(poll_votes_.size()) > poll_option){
...
void Poll::getPollOptions() const {
    for (int i{0} ; i <= int(poll_options_.size() -1); i++) {
